...
export default function TradingMockup() {
  ...
  const [fundingRate, setFundingRate] = useState<string>('0.0000');
  ...
  const [latestCandleTime, setLatestCandleTime] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    // Streams: Ticker, Mark Price (for funding rate), Depth, Aggregated Trades, Kline 1m
    const ws = new WebSocket('wss://fstream.binance.com/stream?streams=btcusdt@ticker/btcusdt@markPrice/btcusdt@depth20@100ms/btcusdt@aggTrade/btcusdt@kline_1m');
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (stream === 'btcusdt@ticker') {
          // Update last price from 24hr ticker
          setLastPrice(parseFloat(data.c).toFixed(1)); // Last price
        } else if (stream === 'btcusdt@markPrice') {
          // Update mark price and funding rate from markPrice stream
          setMarkPrice(parseFloat(data.p).toFixed(1)); // Mark price
          
          // Update funding rate (r = funding rate, T = next funding time)
          if (data.r) {
            const fundingPct = (parseFloat(data.r) * 100).toFixed(4);
            setFundingRate(fundingPct);
          }
          if (data.T) {
            setNextFundingTime(data.T);
          }
          ...
          setRecentTrades((prev: any[]) => [newTrade, ...prev].slice(0, 20));
           ...
           // Update chartData for indicators
           const lastCandle = chartData.current[chartData.current.length - 1];
           if (lastCandle && lastCandle.time === candle.time) {
               // Update last candle
               chartData.current[chartData.current.length - 1] = candle;
           } else {
               // New candle
               chartData.current.push(candle);
           }
           setLatestCandleTime(candle.time);
        }
      } catch (e) {
...
// ... (existing imports)
  ...
  const chartData = useRef<any[]>([]); // Keep track of data for indicators

  // Calculate and Update Indicators
  useEffect(() => {
      // We need the chart instance to add/remove series. 
      // Since we don't have it in a ref, we'll just clear and re-add series if we can access the chart.
      // Actually, we can't easily access the chart instance created in the previous useEffect.
      // We need to refactor to store the chart instance in a ref.
  }, [selectedIndicators, chartData.current]); 
  
  // REFACTOR: Combine chart initialization and indicator logic or use a ref for the chart.
  // Let's use a ref for the chart instance.
    ...
    fetch('https://fapi.binance.com/fapi/v1/klines?symbol=BTCUSDT&interval=1m&limit=1000')
      ...
      .then(data => {
        const cdata = data.map((d: any) => ({
          ...
          color: parseFloat(d[4]) >= parseFloat(d[1]) ? '#0ecb81' : '#f6465d'
        }));
        
        candlestickSeries.setData(cdata.map((d: any) => ({
             time: d.time, open: d.open, high: d.high, low: d.low, close: d.close
        })));
        
        volumeSeries.setData(cdata.map((d: any) => ({
            time: d.time, value: d.value, color: d.color
        })));

        chartData.current = cdata;
      })
  ...
  // Indicator Logic
  useEffect(() => {
    if (!chartRef.current || !chartData.current.length) return;
    ...
    const closes = chartData.current.map((d: any) => d.close);

    selectedIndicators.forEach(ind => {
        if (ind === 'EMA7') {
            const ema7 = EMA.calculate({ period: 7, values: closes });
            const series = chartRef.current.addLineSeries({ color: 'yellow', lineWidth: 1 });
            const data = chartData.current.slice(chartData.current.length - ema7.length).map((d: any, i: number) => ({
                time: d.time,
                value: ema7[i]
            }));
            series.setData(data);
            ...
            const series = chartRef.current.addLineSeries({ color: 'orange', lineWidth: 1 });
            const data = chartData.current.slice(chartData.current.length - ema21.length).map((d: any, i: number) => ({
                time: d.time,
                value: ema21[i]
            }));
            series.setData(data);
            ...
            const series = chartRef.current.addLineSeries({ color: 'purple', lineWidth: 1 });
            const data = chartData.current.slice(chartData.current.length - ema99.length).map((d: any, i: number) => ({
                time: d.time,
                value: ema99[i]
            }));
            series.setData(data);
            ...
            const upper = chartRef.current.addLineSeries({ color: 'rgba(0, 150, 136, 0.5)', lineWidth: 1 });
            const lower = chartRef.current.addLineSeries({ color: 'rgba(0, 150, 136, 0.5)', lineWidth: 1 });
            const middle = chartRef.current.addLineSeries({ color: 'rgba(255, 0, 0, 0.5)', lineWidth: 1 });
            
            const lenDiff = chartData.current.length - bb.length;
            
            upper.setData(bb.map((d: any, i: number) => ({ time: chartData.current[i + lenDiff].time, value: d.upper })));
            lower.setData(bb.map((d: any, i: number) => ({ time: chartData.current[i + lenDiff].time, value: d.lower })));
            middle.setData(bb.map((d: any, i: number) => ({ time: chartData.current[i + lenDiff].time, value: d.middle })));
            
            indicatorSeriesRefs.current.push(upper, lower, middle);
        }
    });

  }, [selectedIndicators, latestCandleTime]); // Re-run when indicators change or data loads
      ...
      <header className="flex h-12 shrink-0 items-center justify-between border-b border-[#2b3139] bg-[#181a20] px-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-4 text-xs">
            <span className={`text-lg font-bold ${parseFloat(lastPrice) >= parseFloat(markPrice) ? 'text-[#0ecb81]' : 'text-[#f6465d]'}`}>
              {lastPrice !== '0.00' ? parseFloat(lastPrice).toLocaleString() : '87,859.7'}
            </span>
            <div className="flex flex-col">
              <span className="text-gray-400">Mark</span>
              <span>{markPrice !== '0.00' ? parseFloat(markPrice).toLocaleString() : '87,859.8'}</span>
            </div>

            <div className="flex flex-col">
              <span className="text-gray-400">Funding / Countdown</span>
              <span className={`${parseFloat(fundingRate) >= 0 ? 'text-[#0ecb81]' : 'text-[#f6465d]'}`}>
                {fundingRate !== '0.0000' ? fundingRate : '0.0001'}% / {countdown}
              </span>
            </div>
